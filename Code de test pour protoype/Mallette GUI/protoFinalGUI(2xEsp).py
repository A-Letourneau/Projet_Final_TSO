...
#Crée un "Widget" de rond de couleur pour l'état des boutons
...
#Change la couleur d'un "Widget" de rond de couleur pour représenter l'état des boutons
...
layout_POT_SW = [
                    ...
                    [sg.Image(source=toggle_btn_off, key="SW1_PIC"), sg.Image(source=toggle_btn_off, key="SW2_PIC"), sg.ProgressBar(maxPotVal, key='POT1'), sg.ProgressBar(maxPotVal, key='POT2'), sg.ProgressBar(maxPotVal, key='POT3')],
                    [sg.Button('Exit')]
                ]
...
while True:  # Event Loop
    ...
    with SMBus(1) as bus:
        msg_BTN = i2c_msg.read(slave_address_BTN, 110)
        bus.i2c_rdwr(msg_BTN)
    ...
    msg_BTN = json.loads(strReceived_BTN) #Transforme la string JSON en dict pour l'utiliser plus facilement
    
    #Demande le json du esp32 et reçoit une liste de byte
    with SMBus(1) as bus:
        msg_POT_SW = i2c_msg.read(slave_address_POT_SW, 110)
        bus.i2c_rdwr(msg_POT_SW)
    #Transforme la ligne de byte en string
