...
"""
Liste de composant et test
- Pi powered avec 5v externe
- Bouton lumineux
- 2 rond de DEL (sudo)
- 3 pot
- 8 sw
...
def draw_axis():
    window_POT['graph'].draw_line((-SIZE_X,0), (SIZE_X, 0))                # axis lines
    window_POT['graph'].draw_line((0,-SIZE_Y), (0,SIZE_Y))

    for x in range(-SIZE_X, SIZE_X+1, NUMBER_MARKER_FREQUENCY):
        window_POT['graph'].draw_line((x,-3), (x,3))                       # tick marks
        if x != 0:
            window_POT['graph'].draw_text( str(x), (x,-10), color='green', font='Algerian 10')      # numeric labels

    for y in range(-SIZE_Y, SIZE_Y+1, NUMBER_MARKER_FREQUENCY):
        window_POT['graph'].draw_line((-3,y), (3,y))
        if y != 0:
            window_POT['graph'].draw_text( str(y), (-10,y), color='blue')
...
def make_winSelect():
    layout_winSelect = [
            ...
            [sg.Button("SW"), sg.Button("POT"), sg.Button("Croco")],
            [sg.Button('Exit')]
         ]
    return sg.Window('Win Select', layout_winSelect, default_element_size=(12, 1), auto_size_text=False, finalize=True) 
...
#window_SW = make_winSW()
window_SW = None
#window_POT = make_winPOT()
window_POT = None
#window_Croco = make_winCroco()
window_Croco = None
...
while True: 
    ...
    if event == 'Exit' or event == sg.WIN_CLOSED: #Si on ferme une fenetre on ferme toutes les fenetres
        ...
        if window == window_SW:
            window_SW = None
        elif window == window_POT:
            window_POT = None
        elif window == window_Croco:
            window_Croco = None
...
#Essaye de lire les json des esp32 et met un message d'erreur s'il n'y parvient pas
   ...
   #-----------Fenêtre Interface Croco-----------#
    if window_Croco and not Crocoerror: #Detecte si la fenetre existe puis detecte si le i2c fonctionne. L'ordre est important car si la fenetre est None, le Crocoerror n'existe pas
    ...
    if window_POT and not POTerror: #Detecte si la fenetre existe puis detecte si le i2c fonctionne. L'ordre est important car si la fenetre est None, le POTerror existe pas
        window_POT['graph'].erase()
        draw_axis()
        ...
        if correctSin:
            ...
            if DEBUG:
                print(amplitudeGoal, periodeGoal, posYGoal)
            correctSin = False

        prev_x = prev_y = None
        for x in range(int(-SIZE_X/2),int(SIZE_X/2)):
            #f(x)=a*sin(p(x))+pY
            y = scale(int(msg_POT['JsonData']['Pot1']), (0, 4096), (0, 50)) * math.sin(scale(int(msg_POT['JsonData']['Pot2']), (0, 4096), (10, 25))/100 * x) + scale(int(msg_POT['JsonData']['Pot3']), (0, 4096), (-10, 10))
            if prev_x is not None:
                window_POT['graph'].draw_line((prev_x, prev_y), (x,y), color='red')
            prev_x, prev_y = x, y
            
            #if amplitudeGoal-MARGINS <= scale(int(msg_POT['JsonData']['Pot1']), (0, 4096), (0, 50)) <= amplitudeGoal+MARGINS and periodeGoal-MARGINS <= scale(int(msg_POT['JsonData']['Pot2'], (0, 4096), (10, 25)) <= periodeGoal+MARGINS and posYGoal-MARGINS <= scale(int(msg_POT['JsonData']['Pot3']), (0, 4096), (-10, 10)) <= posYGoal+MARGINS:
            if amplitudeGoal-MARGINS <= scale(int(msg_POT['JsonData']['Pot1']), (0, 4096), (0, 50)) <= amplitudeGoal+MARGINS:
                if periodeGoal-MARGINS <= scale(int(msg_POT['JsonData']['Pot2']), (0, 4096), (10, 25)) <= periodeGoal+MARGINS:
                    if posYGoal-MARGINS <= scale(int(msg_POT['JsonData']['Pot3']), (0, 4096), (-10, 10)) <= posYGoal+MARGINS:
                        correctSin = True
        ...
        for x in range(int(-SIZE_X/2),int(SIZE_X/2)):
            #f(x)=a*sin(p(x−pX))+pY
            #y = math.sin(x)
            y = amplitudeGoal * math.sin((periodeGoal/100)*(x)) + posYGoal
            if prev_x_goal is not None:
                window_POT['graph'].draw_line((prev_x_goal, prev_y_goal), (x,y), color='black')
            prev_x_goal, prev_y_goal = x, y



    #-----------Fenêtre Interface_SW-----------#
    if window_SW and not SWerror : #Detecte si la fenetre existe puis detecte si le i2c fonctionne. L'ordre est important car si la fenetre est None, le SWerror existe pas
