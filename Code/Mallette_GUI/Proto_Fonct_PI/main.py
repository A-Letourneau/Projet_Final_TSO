...
L'interface SW utilise des images d'interrupteurs pour démontrer l'état des interrupteurs.
...
def make_winSelect():
    layout_winSelect = [
            ...
            [sg.Button("Labyrinthe"), sg.Button("Bouton")],
            [sg.Button('Exit')]
         ]
...
mazeEnigme = mazeClass(mazeFile=open(f"/home/beaulibe/Desktop/Mallette_GUI/Proto_Fonct_PI/maze1.txt"), nbGate=4, SLAVE_ADDRESS_MAZE=SLAVE_ADDRESS_SW, DEBUG=DEBUG, DEL_ACTIVE=DEL_ACTIVE)
my_POT = POT(SLAVE_ADDRESS_POT,DEBUG, strip3)
buttonEnigme = BTN_CLASS(switch, led, DEBUG)
...
while True: 
    ...
    if switch.value:
        ...
        led.value = True
        if DEL_ACTIVE:
            moduleDEL.rainbowCycle(strip4, 0, 1)
        if DEBUG:
            print('BTN on')
    ...
    if event == 'Exit' or event == sg.WIN_CLOSED: #Si on ferme une fenetre on ferme toutes les fenetres
        ...
        elif window == mazeEnigme.window_maze:
