...
class SW_MODULE:
    ...
    #------------------- les variables statiques utilisés par l'énigme des intérupteurs -------------------#
        ...
        if self.window_SW:
            ...
            if not self.SWerror : #Detecte si la fenetre existe puis detecte si le i2c fonctionne. L'ordre est important car si la fenetre est None, le SWerror existe pas
                ...
                for curSw in range(self.NB_SW):
                    if decimalNb > 0:
                        binaryNb = str(decimalNb & 1) + binaryNb
                        decimalNb >>= 1
                    else:
                        binaryNb = "0" + binaryNb
                
                self.window_SW['binaryNb'].update("Nombre binaire : 0b" + str(binaryNb))
