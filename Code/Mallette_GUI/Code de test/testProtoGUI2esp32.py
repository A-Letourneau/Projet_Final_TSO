...
window = sg.Window('Game Interface', layout, default_element_size=(12, 1), auto_size_text=False, finalize=True)

layout2 = [[sg.Text('My LED Status Indicators', size=(20,1))],
          [sg.Text('SW1'),  sg.Text('SW2'),  sg.Text('BTN1')],
          [sg.Image(source=toggle_btn_off, key="SW1_PIC"), sg.Image(source=toggle_btn_off, key="SW2_PIC"), LEDIndicator('BTN1')],
          [sg.Button('Exit')]]


window2 = sg.Window('Game Interface2', layout, default_element_size=(12, 1), auto_size_text=False, finalize=True)
...
while True:  # Event Loop
    ...
    with SMBus(1) as bus:
        msg = i2c_msg.read(slave_address1, 110)
        bus.i2c_rdwr(msg)
    ...
    if sendMQTT > 20:
        print("MQTT SENT")
        waitingMQTT = client.publish('CegepSherbrooke/projet/tso/Mallette', payload=strReceived, qos=0, retain=False)
    ...
    if msg['JsonData']['sw1'] == '1':
        window["SW1_PIC"].update(source=toggle_btn_on)
    else:
        window["SW1_PIC"].update(source=toggle_btn_off)


    if msg['JsonData']['sw2'] == '1':
        window["SW2_PIC"].update(source=toggle_btn_on)
    else:
        window["SW2_PIC"].update(source=toggle_btn_off)


    SetLED(window, 'BTN1', 'green' if msg['JsonData']['BTN1'] == '1' else 'red')
window.close()
