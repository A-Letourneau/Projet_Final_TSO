#!/home/pi/path/to/venv/bin/python3
"""
...
from test_inter_Croco import Croco
import I2c_Comm
from test_inter_SW import SW_MODULE
from test_inter_POT import POT
from test_mazeModule import mazeClass
...
def make_winSelect():
    layout_winSelect = [
            [sg.Text("Veuillez selectionner l'enigme desire", size=(35,1), key = "titleSW")],
            [sg.Button("SW"), sg.Button("POT"), sg.Button("Croco")],
            [sg.Button("Labyrinthe")],
            [sg.Button('Exit')]
         ]
    return sg.Window('Fenetre de selection', layout_winSelect, default_element_size=(12, 1) , auto_size_text=False, finalize=True) 
...
#create te master window
layout_SW=my_SW.Make_WinSW()
layout_POT=my_POT.Make_WinPOT()
layout_Croco=my_Croco.Make_WinCroco()
layout_Maze=mazeEnigme.startMaze()
BigButton = [[sg.Button('End', key='EndButton', disabled = True)]]


Main_layout = [[sg.Column(layout_Croco, key='-COL1-', background_color = 'red',element_justification='center', size=(474,225)),sg.Push(), sg.Column(layout_POT, visible=True, key='-COL2-',background_color = 'blue',element_justification='center')],
               [sg.Push(),sg.Column(BigButton,element_justification='center'),sg.Push()],
          [ sg.Column(layout_Maze, visible=True, key='-COL3-',background_color='green',element_justification='center'), sg.VPush(),sg.Push(), sg.Column(layout_SW, visible=True, key='-COL4-',background_color = 'yellow',element_justification='center')]]
window = sg.Window('Swapping the contents of a window', Main_layout,return_keyboard_events=True, resizable= True, finalize = True)

#donne l adress de la fenetre a toutes les enigmes 
my_SW.window_SW =window
my_POT.window_POT =window
my_Croco.window_Croco =window
mazeEnigme.window_maze =window
move = ''
...
while True: 
    ...
    if switch.value:
        ...
        led.value = True
        if DEL_ACTIVE:
            moduleDEL.rainbowCycle(strip3, 0, 1)
        if DEBUG:
            print('BTN on')
        
        
    #-------------Lecture des boutons des fenetres--------------#
    event, value = window.read(timeout = REFRESH_RATE)
    ...
    if event != "__TIMEOUT__":#and event.isalpha():	#
        ...
        move = event.upper()
        move = move[0]
        mazeEnigme.window_maze["input"].update(move)
        
        
        mazeEnigme.deplacement(move)
